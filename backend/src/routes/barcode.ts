...
function shortlistQuery(off: OffProduct): string {
  const tags = (off.categories_tags ?? []).map((t) =>
    t.replace(/^[a-z]{2}:/, '').replace(/-/g, ' '),
  );
  return [off.product_name ?? '', ...tags].join(' ');
}

export function createBarcode(usage: UsageService) {
  ...
  barcode.get('/:ean', async (c) => {
    ...
    if (hit && Date.now() - hit.at < hit.ttl) {
      const cached = hit.body as { coverage_miss?: boolean };
      if (cached.coverage_miss === false) {
        const failed = await consumeAnalysis(c, usage, authorization, 'barcode', ean);
    ...
    try {
      const res = await fetch(`${OFF_URL}/${ean}.json?fields=${OFF_FIELDS}`, {
        ...
        headers: {
          'User-Agent': 'Drop/0.1 (water footprint tracker; hataewook12@gmail.com)',
        },
      });
      if (res.ok) {
    ...
    if (off) {
      ...
      const shortlist =
        mapping.catalog_id || !off.product_name
          ? []
            ...
            messages: [
              ...
              },
              {
                ...
                content:
                  ...
                  shortlist.map((e) => `${e.catalog_id}|${e.display_name}`).join('\n'),
              },
            ],
    ...
    cache.set(ean, {
      at: Date.now(),
      ttl: offStatus === 'unavailable' ? UNAVAILABLE_TTL : TTL,
      body,
    });
    if (!body.coverage_miss) {
      const failed = await consumeAnalysis(c, usage, authorization, 'barcode', ean);
      if (failed) return failed;
    }
    return c.json(body);
